...
void Game::UpdatePhysics()
{
    ...
    phyWorld->ClearForces(); // Limpiar las fuerzas aplicadas a los cuerpos
     // Dibujar el mundo físico para depuración
}
...
void Game::InitPhysics()
{
    ...
    /*
    b2Body* fallingRect = Box2DHelper::CreateRectangularDynamicBody(phyWorld, 10, 10, 1.0f, 0.8f, 0.1f);
    fallingRect->SetTransform(b2Vec2(40.0f, 40.0f), 0.0f);

    b2Body* WallBody = Box2DHelper::CreateRectangularStaticBody(phyWorld, 10, 100);
    WallBody->SetTransform(b2Vec2(75.0f, 70.0f), 0.7f);
*/

    b2Vec2 vertices[3];
    vertices[0].Set(0.0f, 0.0f);
    vertices[1].Set(25.0f, 10.0f);
    vertices[2].Set(60.0f, 20.0f);

    b2Body* inclinedPlane = Box2DHelper::CreatePolyStaticBody(phyWorld, vertices, 3);
    inclinedPlane->SetTransform(b2Vec2(20.0f, 70.0f), 0.0f);

    b2Fixture* fixIncline = inclinedPlane->GetFixtureList();
    fixIncline->SetFriction(0.8);

    // Crear un círculo que se controlará con el teclado

    controlBody = Box2DHelper::CreateRectangularDynamicBody(phyWorld, 10, 10, 1.0f, 0.3f, 0.1f);
    controlBody->SetTransform(b2Vec2(50.0f, 50.0f), 0.001f);
}
